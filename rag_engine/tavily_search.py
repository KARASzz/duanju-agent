...
class TavilySearcher:
    ...
    def __init__(self, api_key: str = None):
        ...
        if not self.api_key:
            print("⚠️ [Tavily] 未配置 API Key，网络检索功能已禁用。")
    ...
    def search_hot_trends(self, query: str, max_results: int = 4) -> List[Dict]:
        ...
        try:
            print(f"🔍 [Tavily 智能雷达] 正在扫描外网全域关于【{query}】的最新趋势与设定点...")
            ...
            print(f"❌ [Tavily] 认证或配额失败 (常见于 Key 错误): {e}")
            return []
        except Exception as e:
            print(f"❌ [Tavily] 网络连接检索异常: {e}")
            return []
...
if __name__ == "__main__":
    ...
    if searcher.api_key:
        ...
        print("💡 [未配置 API Key] 请在系统环境中设置 `TAVILY_API_KEY` 来解锁实时的外网爆款挖掘能力。")
